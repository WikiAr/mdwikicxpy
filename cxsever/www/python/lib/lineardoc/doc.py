...
class Doc:
        ...
        for i, item in enumerate(self.items):
            if item['type'] == 'open':
                ...
                if tag.get('attributes', {}).get('id'):
                    ...
                    if (tag['name'] in ['h1', 'h2', 'h3', 'h4', 'h5'] and
                            i + 1 < len(self.items) and
                            self.items[i + 1]['type'] == 'textblock'):
                        h = hashlib.sha256()
                    ...
                    if (i + 1 < len(self.items) and 
                        self.items[i + 1]['type'] == 'open' and
                        self.items[i + 1]['item'].get('name') == 'h2'):
                        section_number += 1
                ...
                new_doc.add_item(
                    'textblock',
                    text_block.segment(get_boundaries, get_next_id) if (text_block.can_segment and not transclusion_context)
    ...
    def get_html(self):
        ...
        for item in self.items:
            ...
            if (isinstance(item_obj, dict) and 
                item_obj.get('attributes', {}).get('class') == 'cx-segment-block'):
                continue
