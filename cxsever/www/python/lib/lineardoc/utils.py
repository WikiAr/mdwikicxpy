...
def is_reference(tag):
    ...
    """
    if ((tag['name'] == 'span' or tag['name'] == 'sup') and
            tag.get('attributes', {}).get('typeof') == 'mw:Extension/ref'):
        return True
    elif (tag['name'] == 'sup' and
            tag.get('attributes', {}).get('class') == 'reference'):
        return True
    return False
...
def is_math(tag):
    ...
    """
    return ((tag['name'] == 'span' or tag['name'] == 'sup') and
            tag.get('attributes', {}).get('typeof') == 'mw:Extension/math')
...
def is_gallery(tag):
    ...
    return (tag['name'] == 'ul' and
            tag.get('attributes', {}).get('typeof') == 'mw:Extension/gallery')


def is_reference_list(tag):
    """Check if tag is a reference list."""
    return (tag['name'] == 'div' and
            tag.get('attributes', {}).get('typeof') == 'mw:Extension/references' and
            tag.get('attributes', {}).get('data-mw'))
...
def is_segment(tag):
    ...
    return (tag['name'] == 'span' and
            tag.get('attributes', {}).get('class') == 'cx-segment')
...
def is_transclusion_fragment(tag):
    ...
    return (cxutil.get_prop(['attributes', 'about'], tag) and
            not cxutil.get_prop(['attributes', 'data-mw'], tag))
...
def add_common_tag(text_chunks, tag):
    ...
    for text_chunk in text_chunks:
        ...
        new_text_chunks.append(TextChunk(
            text_chunk.text,
            new_tags,
...
def set_link_ids_in_place(text_chunks, get_next_id):
    ...
    for text_chunk in text_chunks:
        for tag in text_chunk.tags:
            if (tag['name'] == 'a' and
                    tag.get('attributes', {}).get('href') is not None and
                    tag.get('attributes', {}).get('rel') and
                    f" {tag['attributes']['rel']} ".find(' mw:WikiLink ') != -1 and
                    tag.get('attributes', {}).get('data-linkid') is None):
    ...
    for i in range(1, len(section_doc.items)):
        ...
        if item_type == 'close':
            if block_stack:
                ...
                if (current_close_tag and
                        len(block_stack) == 0 and
                        ((is_transclusion(current_close_tag) and
                          current_close_tag.get('attributes', {}).get('about') ==
                          first_block_template.get('attributes', {}).get('about')) or
