...
class Builder:
    ...
    def is_category(self, tag):
        ...
        return (tag['name'] == 'link' and
                f" {rel} ".find(' mw:PageProp/Category ') != -1 and
                not tag.get('attributes', {}).get('about'))
    ...
    def pop_block_tag(self, tag_name):
        ...
        if not tag or tag['name'] != tag_name:
            raise Exception(
                f"Mismatched block tags: open={tag['name'] if tag else None}, close={tag_name}"
            )
    ...
    def pop_inline_annotation_tag(self, tag_name):
        ...
        if not tag or tag['name'] != tag_name:
            raise Exception(
                f"Mismatched inline tags: open={tag['name'] if tag else None}, close={tag_name}"
            )
